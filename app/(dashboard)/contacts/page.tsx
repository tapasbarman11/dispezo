    ...
    <div className="glass rounded-2xl shadow-[var(--shadow-card)] overflow-hidden">
      ...
      <div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-muted/40"><tr>{["","Name","Phone","Email","Segment","Source","Added",""] .map((h,i)=><th key={i} className={`px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-muted-foreground ${i===7?"text-right":""}`}>{i===0?<input type="checkbox" checked={allSelected} onChange={e=>setSelected(e.target.checked?contacts.map(c=>c.id):[])}/>:h}</th>)}</tr></thead><tbody className="divide-y divide-border">
        {loading ? (
          ...
          contacts.map(c=><tr key={c.id} className="hover:bg-muted/30">
            <td className="px-5 py-4"><input type="checkbox" checked={selected.includes(c.id)} onChange={e=>setSelected(s=>e.target.checked?[...s,c.id]:s.filter(x=>x!==c.id))}/></td>
            <td className="px-5 py-4"><div className="flex items-center gap-3"><div className="size-8 rounded-full gradient-brand grid place-items-center text-white text-[10px] font-semibold">{initials(c.name)}</div><span className="text-sm font-medium whitespace-nowrap">{c.name||"Unnamed"}</span></div></td>
            <td className="px-5 py-4 text-sm font-mono text-muted-foreground">{c.phone}</td>
            <td className="px-5 py-4 text-sm text-muted-foreground">{c.email||"—"}</td>
            <td className="px-5 py-4">{c.tag?<span className={`px-2 py-1 rounded-full text-[10px] font-bold ${tagClass(c.tag)}`}>{c.tag}</span>:"—"}</td>
            <td className="px-5 py-4 text-xs text-muted-foreground">{c.source||"—"}</td>
            <td className="px-5 py-4 text-xs text-muted-foreground whitespace-nowrap">{new Date(c.createdAt).toLocaleDateString("en-IN")}</td>
            <td className="px-5 py-4 text-right"><div className="flex justify-end gap-1"><button onClick={()=>openEdit(c)} className="p-2 rounded-lg hover:bg-muted"><Pencil className="size-4"/></button><button onClick={()=>remove([c.id])} className="p-2 rounded-lg text-red-500 hover:bg-red-50"><Trash2 className="size-4"/></button></div></td>
          </tr>)
        )}
      </tbody></table></div>

...
export default function WhatsAppPage() {
    ...
    const templateButtons = useMemo(() => {
        ...
        if (typeof currentTemplate.buttons === "string") {
            try {
                return JSON.parse(currentTemplate.buttons);
            } catch {
                return [];
            }
        }
    ...
    const loadLocalTemplates = useCallback(async () => {
        try {
            ...
            console.error("Unable to load local templates:", err);
            return [];
        }
    }, []);
    ...
    const sendTestMessage = useCallback(async () => {
        ...
        try {
                ...
                if (uploadJson.success) {
                    components.push({
                        ...
                        parameters: [{
                            type: "image",
                            image: {
                                link: `${window.location.origin}${uploadJson.path}`,
                            },
                        }],
                    });
                }
            ...
            alert(
                "Message submitted to WhatsApp. Check Recent Activity for delivery status."
            );
        } catch (err: any) {
            alert(err.message);
        } finally {
                    ...
                    <div className="grid grid-cols-12 gap-4 items-center">
                        ...
                        <div className="col-span-3 flex flex-col justify-center pl-3">
                            ...
                            {!statusLoaded ? (
                                ...
                                <span
                                    className={`inline-flex w-fit items-center rounded-full px-3 py-1 text-xs font-semibold ${qualityColor()}`}
                                >
                                    {connection.qualityRating}
                                </span>
                            ) : (
                    ...
                    >
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                {templates.length === 0 && activityLoaded && (
                            ...
                            <div className="mb-4 space-y-2 rounded-lg border border-amber-200 bg-amber-50 p-3">
                                ...
                                {templateVars.map((v: string) => (
                                    <div key={v} className="flex items-center gap-2">
                                        <span className="w-10 shrink-0 text-xs font-mono text-amber-600">
                                            {`{{${v}}}`}
                                        </span>
                                        <input
                        ...
                        {templateButtons.length > 0 && (
                            <div className="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-3">
                                <p className="text-[11px] font-medium text-blue-700 mb-1.5">
                                    Action buttons in this template
                                </p>
                                ...
                                {templateButtons.map((btn: any, i: number) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {t === "PHONE_NUMBER" && btn.phoneNumber && (
                        ...
                        <div className="mb-4">
                            ...
                            <div className="flex items-start justify-between rounded-xl border border-emerald-200 bg-[#E7FFEE] px-4 py-4">
                                <div className="flex-1 pr-3">
                                    {selectedTemplate ? (
                                        <>
                                            <div className="text-[10px] font-semibold uppercase text-emerald-700 mb-1.5">
                                                {selectedTemplate}
                                            </div>
                        ...
                        <motion.button
                            ...
                            whileTap={{ scale: 0.98 }}
                            disabled={sending || !recipientNumber.trim() || !selectedTemplate}
                            onClick={sendTestMessage}
                            className="gradient-brand flex h-12 w-full items-center justify-center gap-2 rounded-xl px-6 text-sm font-semibold text-white shadow-[var(--shadow-glow)] transition-shadow hover:shadow-xl disabled:opacity-60"
                        >
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-border bg-muted/30">
                                        {["Time", "Recipient", "Template", "Status"].map((h) => (
                                            <th
                                                key={h}
                                                className="px-5 py-3 text-left text-[10px] font-semibold uppercase tracking-widest text-slate-400 whitespace-nowrap"
                                            >
                                                {h}
                                            </th>
                                        ))}
                                    ...
                                    {!activityLoaded ? (
                                        ...
                                        paginatedActivity.map((row: any) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-5 py-3">
                                                    <code className="text-[11px] text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded">
                                                        {row.template}
                                                    </code>
                                                </td>
                                                <td className="px-5 py-3">

...
function NavItem({
  ...
  return (
    <Link
      href={to}
      className={`group flex items-center gap-3 px-4 py-3 rounded-2xl text-[15px] font-medium transition-all duration-300 ${
        nested ? "ml-4" : ""
      } ${
        active
          ? "bg-white text-brand-purple shadow-lg border border-white"
          : "text-slate-700 hover:translate-x-1 hover:bg-brand-purple/10 hover:text-brand-purple hover:shadow-md hover:border hover:border-brand-purple/30"
      }`}
    >
      ...
      <aside className="fixed inset-y-0 left-0 z-40 w-72 flex flex-col bg-gradient-to-b from-brand-purple/15 via-brand-blue/5 to-white border-r border-slate-200 overflow-hidden">
        ...
        <div className="relative flex justify-center px-6 pt-8 pb-6">
          <img
            src={BRAND.logo}
            alt={BRAND.name}
            className="w-44 object-contain transition-transform duration-300 hover:scale-105"
          />
        </div>

        <nav className="relative flex-1 px-4 py-3 space-y-2">
          {topNav.map((item) => (
            <NavItem key={item.to} {...item} path={path} />
          ))}
          ...
          <div className="pt-1">
            ...
            <div className="relative mt-1 ml-5 pl-0 border-l border-brand-purple/20 space-y-1">
        ...
        <div className="relative p-4 border-t border-white/10">
          <button
            ...
            className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl bg-white/80 border border-slate-200 text-slate-700 shadow-sm hover:bg-red-50 hover:text-red-600 transition-all duration-300"
          >
      ...
      <div className="flex-1 ml-72 min-w-0">
        <header className="sticky top-0 z-30 h-20 px-8 flex items-center justify-between border-b border-slate-200/70 bg-white/80 backdrop-blur-2xl">
          <div className="flex items-center">
            <Link
              href="/pricing"
              className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-600 transition hover:text-brand-purple"
            >
